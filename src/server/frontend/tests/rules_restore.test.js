  ...
  AgentGuardRuleDSL: {
    ...
    serializeRule(rule) {
      ...
      if (Array.isArray(rule.phases) && rule.phases.length) {
        lines.push(`PHASES: ${rule.phases.join(", ")}`);
      }
      if (rule.onClause) {
...
test("parsePublishedRuleSource restores tool-after rules with direct ON clauses", () => {
  const restored = parsePublishedRuleSource([
  ...
  assert.ok(restored);
  assert.deepEqual(restored.phases, ["tool_after"]);
  assert.equal(restored.onClause, "tool_call(http.post)");
});
...
test("buildRuleListSource renders published DSL preview text", () => {
  const preview = buildRuleListSource({
    rule_id: "published_one",
    action: "HUMAN_CHECK",
    source: [
  ...
  assert.match(preview, /^PHASES: tool_before$/m);
