...
@dataclass(frozen=True)
class TicketSessionAgentIdentity:
...
class DifyAuthBroker:
    ...
    ) -> tuple[Any, list[AgentBootstrapResult]]:
    ...
    def bootstrap_opencode_agents(
        ...
        *,
        ...
        for item in agents:
            ...
            agent_type = _optional_text(item.get("agent_type")) or "agent"
            if agent_type != "agent":
                raise BadAuthRequest("OpenCode bootstrap only supports agent_type=agent")
            item_provider_instance_id = _optional_text(item.get("provider_instance_id")) or _optional_text(provider_instance_id)
            item_tenant_id = _optional_text(item.get("tenant_id")) or _optional_text(tenant_id)
            item_metadata = {
                **clean_metadata,
                **(item.get("metadata") if isinstance(item.get("metadata"), dict) else {}),
                "external_agent_id": external_agent_id,
                "opencode_agent_id": external_agent_id,
                "ticket_prefix": identity.ticket_prefix,
            }
            registration = self.agent_store.register_agent(
                provider="opencode",
        ...
        consume_agent_id = results[0].agent.agent_id if results else "opencode-bootstrap"
        self._consume_user_ticket(
            user_ticket,
            agent_id=consume_agent_id,
            session_id=f"opencode-bootstrap:{_optional_text(provider_instance_id) or 'default'}",
        )
        return identity, results
    ...
    def create_opencode_session(
        ...
        *,
        ...
        user_ids = self.agent_store.user_ids_for_agent(agent_id)
        if not user_ids:
            raise RuntimeAuthForbidden("OpenCode agent is not bound to an AgentGuard user")
        if len(user_ids) > 1:
            raise RuntimeAuthForbidden("OpenCode agent is bound to multiple AgentGuard users")
        user_id = next(iter(user_ids))
        verification = self._verify_proof(
            dpop_proof,
            method=method,
        ...
        if existing is not None:
            if existing.user_id != user_id:
                raise RuntimeAuthForbidden("OpenCode session belongs to another AgentGuard user")
            if existing.status != "active":
                raise RuntimeAuthUnauthorized("OpenCode external session is closed")
            if existing.dpop_jkt != verification.jkt:
                raise RuntimeAuthForbidden("OpenCode session is bound to another DPoP key")
            self.session_store.touch_session(existing.session_id)
            return self._issue_token(existing)
        session = self.session_store.create_session(
            agent_id=agent_id,
    ...
    def create_ticket_session(
        ...
        *,
        ...
        provider = _clean_provider(provider)
        if provider not in {"langchain", "openclaw", "opencode"}:
            raise BadAuthRequest("only provider=langchain, provider=openclaw, or provider=opencode is supported for ticket session create")
        identity = self._resolve_user_ticket(user_ticket)
        verification = self._verify_proof(
            dpop_proof,
            method=method,
        ...
        agent_identity = _ticket_session_agent_identity(provider, clean_metadata, identity.ticket_id)
        registration = self.agent_store.register_agent(
            provider=provider,
            provider_instance_id=None,
            tenant_id=None,
            external_agent_id=agent_identity.external_agent_id,
            ...
            metadata={
                **clean_metadata,
                "external_agent_id": agent_identity.external_agent_id,
                "display_agent_id": agent_identity.display_agent_id,
                "agent_type": agent_identity.agent_type,
                "ticket_prefix": identity.ticket_prefix,
            },
        )
...
def _runtime_agent_name(provider: str) -> str:
    ...
    if provider == "opencode":
        return "OpenCode runtime agent"
    return "LangChain runtime agent"


def _ticket_session_agent_identity(
    provider: str,
    metadata: dict[str, Any],
    ticket_id: int,
) -> TicketSessionAgentIdentity:
    if provider == "opencode":
        opencode_agent = _optional_text(metadata.get("opencode_agent")) or "default"
        external_agent_id = (
            _optional_text(metadata.get("external_agent_id"))
            or f"opencode:{_slug_fragment(opencode_agent)}"
        )
        display_agent_id = _optional_text(metadata.get("display_agent_id")) or external_agent_id
        metadata["opencode_agent"] = opencode_agent
        metadata["external_agent_id"] = external_agent_id
        metadata["display_agent_id"] = display_agent_id
        metadata["agent_type"] = "agent"
        return TicketSessionAgentIdentity(
            external_agent_id=external_agent_id,
            display_agent_id=display_agent_id,
            agent_type="agent",
            name=_optional_text(metadata.get("name")) or f"OpenCode {opencode_agent}",
            description=_optional_text(metadata.get("description")) or f"OpenCode agent '{opencode_agent}' guarded by AgentGuard.",
        )

    external_agent_id = f"ticket-{ticket_id}-{secrets.token_urlsafe(12)}"
    return TicketSessionAgentIdentity(
        external_agent_id=external_agent_id,
        display_agent_id=external_agent_id,
        agent_type="runtime",
        name=_optional_text(metadata.get("name")) or _runtime_agent_name(provider),
...
def _expired(value: datetime) -> bool:
    ...
    return dt.astimezone(UTC).timestamp() <= datetime.now(UTC).timestamp()
