...
def _append_message_item(
    ...
    if role in {"system", "developer"}:
        ...
        history_lines.append(_format_tool_result_line(tool_name or "tool", text))
    else:
...
def _append_tool_result(
    ...
    if error not in (None, ""):
        ...
        history_lines.append(_format_tool_result_line(tool_name, _extract_text(payload.get("result"))))
    return tool_name


def _format_tool_result_line(tool_name: str, text: str) -> str:
    if _is_agentguard_denied_result(text):
        return f"[TOOL_CALL: {tool_name}] Tool was denied by AgentGuard and was not executed."
    return f"[TOOL_RESULT: {tool_name}] {text}"


def _is_agentguard_denied_result(text: str) -> bool:
    parsed = _parse_structured_payload(text)
    if not isinstance(parsed, dict):
        return False
    agentguard = str(parsed.get("agentguard") or "").strip().lower()
    decision = str(parsed.get("decision") or "").strip().lower()
    return agentguard == "blocked" or decision == "deny"
