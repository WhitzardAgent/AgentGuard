"""Server checker class registry and registration decorator."""
...
from backend.runtime.checkers.base import BaseChecker
...
def register(name: str, description: str) -> Callable[[type[BaseChecker]], type[BaseChecker]]:
    """Register a server checker class under a config-friendly name."""
    if not name:
...
def discover_checkers(package_name: str = "backend.runtime.checkers") -> None:
