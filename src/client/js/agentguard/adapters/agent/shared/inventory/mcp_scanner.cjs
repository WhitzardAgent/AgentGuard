...
function scanMcpConfigFile(configPath, options = {}) {
  ...
  return scanMcpServerMap(extracted.servers, {
    ...resolved,
...
function scanMcpServerMap(serverMap, options = {}) {
  const resolved = normalizeOptions(options);
  ...
  const configKey = nonEmptyString(options.configKey);
  const diagnostics = [];
  const servers = [];
  const entries = serverMap && typeof serverMap === "object" && !Array.isArray(serverMap)
  ...
  if (entries.length === 0) {
    if (configPath) {
      diagnostics.push({
        ...
        message: "MCP config did not contain MCP server entries.",
      });
    }
...
function scanMcpServerDescriptor(serverName, serverConfig, options = {}) {
  ...
  const command = normalizeCommand(serverConfig, configDir);
  const source = remote
    ? remoteSourceMetadata(serverConfig, transport)
    : inferLocalSource(command, configDir);
  ...
  return {
    object_type: "mcp",
    source_framework: resolved.sourceFramework,
    name,
    description,
    transport,
    remote,
...
function inferLocalSource(commandConfig, configDir) {
  ...
  const runtimeCommand =
    LOCAL_COMMANDS.has(commandBase) ||
    /^python(?:\d+(?:\.\d+)*)?$/.test(commandBase);
  const entryFromCommand = !runtimeCommand && localCommandPath && isRegularReadableFile(localCommandPath)
    ? localCommandPath
    : "";
  const entryFromArgs = findLocalScriptArg(args, cwd, commandBase);
  const entryFile = entryFromArgs || entryFromCommand;
...
function extractServerMap(parsed) {
  if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
    ...
    if (parsed.mcp && typeof parsed.mcp === "object" && !Array.isArray(parsed.mcp)) {
      return { key: "mcp", servers: parsed.mcp };
    }
  }
  return { key: "", servers: {} };
}
...
function sanitizeServerConfig(serverConfig, configDir) {
  ...
  const environment = serverConfig.environment || serverConfig.env;
  if (environment && typeof environment === "object" && !Array.isArray(environment)) {
    out.env_keys = Object.keys(environment).sort();
  ...
  if (serverConfig.oauth !== undefined) {
    out.oauth = serverConfig.oauth === false ? false : "[redacted]";
  }
  return out;
}

function normalizeCommand(serverConfig, configDir) {
  const rawCommand = serverConfig && serverConfig.command;
  const commandParts = Array.isArray(rawCommand)
    ? rawCommand.map(String).filter((item) => item.trim())
    : [];
  const command = commandParts.length
    ? commandParts[0]
    : nonEmptyString(rawCommand);
  const args = commandParts.length
    ? commandParts.slice(1)
    : Array.isArray(serverConfig && serverConfig.args)
      ? serverConfig.args.map(String)
      : [];
  return {
    command,
    args,
    cwd: resolveOptionalPath(serverConfig && serverConfig.cwd, configDir) || configDir,
  };
}
...
function normalizeOptions(options = {}) {
  ...
  return {
    ...
    sourceFramework: nonEmptyString(input.sourceFramework) || "mcp_native",
  };
}
...
function resolveLocalPath(value, cwd) {
  ...
  if (
    ...
    !text.includes("/") &&
    !text.includes("\\") &&
    !SCRIPT_EXTENSIONS.has(path.extname(text).toLowerCase())
  ) {
    return "";
  }
