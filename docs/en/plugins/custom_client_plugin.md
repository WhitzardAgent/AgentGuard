...
- `decision_candidate`: optional `GuardDecision` recommendation. Use it when the plugin wants to propose `ALLOW` (allow immediately), `DENY` (block execution), or `LOG_ONLY` (annotate it in audit logs but still allow it).
