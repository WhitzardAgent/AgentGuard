...
Plugin 配置按 phase 组织。每个 phase 可以定义由客户端加载的 `local` plugins，以及由服务端加载的 `remote` plugins。每个 plugin 条目都是一个 spec 对象，例如 `{"name": "rule_based_check", "env": {}}`。实现级细节见 [AgentGuard Plugins](plugins.md)。

## 策略

策略是用户定义的控制规则。在内置流程中，这些 DSL 策略由服务端 `rule_based_check` plugin 消费，用于说明某个运行时动作在什么条件下应该被允许、拒绝或转入审核。
...
策略会与 plugin 协同工作：`rule_based_check` 负责评估显式访问控制规则，其他 plugin 可以附加风险信号或给出额外的候选决策。
