...
AgentGuard 是一套面向 AI Agents 的零信任安全防护基座。它可以集成到现有智能体框架中，在智能体运行全流程中提供可配置的安全防护能力：每次调用大模型前、大模型输出后、工具调用前、工具执行完成后，都可以进行识别、拦截、升级处理或记录。同时，AgentGuard 也支持通过可插拔 custom auditor 对已存储运行轨迹进行事后审计。
...
- **Custom auditors**：对已存储 trace 做事后分析，支持复核、合规与事故排查。
